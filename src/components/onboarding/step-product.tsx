...
export function StepProduct({
  ...
  return (
    <div className="py-2">
      <h2 className="font-display text-xl font-bold text-[var(--text-primary)]">Paso 2 de 3: Tu primer producto</h2>
      <p className="mt-1 text-sm text-slate-500">¿Qué vendes? Agrega al menos un producto o servicio.</p>

      <div className="mt-6 space-y-4">
        <div>
          <label className="mb-1.5 block text-sm font-medium text-[var(--text-primary)]">Nombre *</label>
          <Input value={productName} onChange={(e) => onChangeName(e.target.value)} placeholder="Nombre del producto" className="h-12" />
        </div>
        <div>
          <label className="mb-1.5 block text-sm font-medium text-[var(--text-primary)]">Precio de venta ({currency}) *</label>
